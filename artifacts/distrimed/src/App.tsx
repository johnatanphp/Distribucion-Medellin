...
.nav-btn{display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:8px;font-size:.875rem;font-weight:500;color:var(--t1);cursor:pointer;transition:all .16s;margin:1px 0;background:none;width:100%;text-align:left}
...
.stat{background:var(--bg2);border:1px solid var(--bd);border-radius:var(--rl);padding:16px 18px}
...
.prod-thumb{height:160px;display:flex;align-items:center;justify-content:center;font-size:5rem;background:linear-gradient(135deg,var(--bg3) 0%,var(--bg4) 100%);position:relative;overflow:hidden}
...
.prod-badge-sale{position:absolute;top:10px;right:10px;z-index:2}
...
.demo-box b{color:var(--cy);font-family:var(--mono)}
...
.store-hero-bg{height:160px;display:flex;align-items:center;justify-content:center;font-size:6rem;position:relative}
...
const DB = {
  ...
  products: [
    ...
    { id:"p28",storeId:"s11",name:"Chaqueta Vintage 90s",price:195000,category:"Chaquetas",stock:15,rating:4.8,emoji:"🎭",description:"Chaqueta vintage auténtica años 90, pieza única",sales:34,minStock:3,brand:"Vintage Original",colors:["Roja","Verde","Azul"],sizes:["M","L","XL"],isNew:false,discount:0 },
  ...
  ratings: [
    { id:"r1",productId:"p1",userId:"u5",score:5,comment:"Llegó perfecto, el estampado es hermoso" },
    { id:"r2",productId:"p9",userId:"u5",score:5,comment:"El mejor hoodie que he comprado" },
    { id:"r3",productId:"p5",userId:"u6",score:5,comment:"Excelente calidad premium, vale cada peso" },
    { id:"r4",productId:"p12",userId:"u7",score:4,comment:"Súper cómodos" },
    { id:"r5",productId:"p25",userId:"u6",score:5,comment:"Perfecta para la oficina" },
  ],
  orders: [
    { id:"o1",storeId:"s1",storeName:"DistriModa El Norte",userId:"u5",items:[{productId:"p1",name:"Vestido Floral Midi",qty:1,price:89000},{productId:"p2",name:"Blusa Manga Larga Seda",qty:2,price:65000}],total:219000,status:"delivered",createdAt:"2025-02-01" },
    { id:"o2",storeId:"s2",storeName:"Fashion Poblado",userId:"u6",items:[{productId:"p5",name:"Blazer Estructurado Premium",qty:1,price:320000}],total:320000,status:"delivered",createdAt:"2025-02-05" },
    { id:"o3",storeId:"s3",storeName:"Belen Urbano",userId:"u5",items:[{productId:"p9",name:"Hoodie Oversized Logo",qty:1,price:145000},{productId:"p11",name:"Gorra Dad Hat Premium",qty:2,price:78000}],total:301000,status:"processing",createdAt:"2025-03-01" },
    { id:"o4",storeId:"s1",storeName:"DistriModa El Norte",userId:"u6",items:[{productId:"p3",name:"Jean Mom Fit",qty:1,price:128000}],total:128000,status:"shipped",createdAt:"2025-03-15" },
    { id:"o5",storeId:"s9",storeName:"La Floresta Chic",userId:"u7",items:[{productId:"p23",name:"Top Crop Floral",qty:3,price:55000}],total:165000,status:"pending",createdAt:"2025-03-20" },
    { id:"o6",storeId:"s10",storeName:"Masculino Bello",userId:"u5",items:[{productId:"p25",name:"Camisa Oxford Premium",qty:2,price:118000},{productId:"p26",name:"Polo Slim Fit",qty:1,price:85000}],total:321000,status:"pending",createdAt:"2025-03-22" },
  ],
  activity: [
    { type:"order", msg:"Nuevo pedido en Masculino Bello — $321.000", time:"hace 5 min", color:"var(--cy)" },
    { type:"store", msg:"Sport Zone San Javier desactivada", time:"hace 12 min", color:"var(--re)" },
    { type:"user", msg:"Nuevo cliente: Valentina Restrepo registrada", time:"hace 25 min", color:"var(--gr)" },
    { type:"order", msg:"Pedido entregado en El Norte — $219.000", time:"hace 1h", color:"var(--gr)" },
    { type:"stock", msg:"⚠️ Stock bajo: Bolso Tote Canvas (3 uds)", time:"hace 2h", color:"var(--ye)" },
    { type:"order", msg:"Nuevo pedido en La Floresta — $165.000", time:"hace 3h", color:"var(--cy)" },
    { type:"store", msg:"Nueva tienda creada: Lencería Santa Fe", time:"hace 5h", color:"var(--pu)" },
  ],
  salesByMonth:[42,58,71,65,89,103,92,115,128,144,136,158],
  templates: [
...
const statusMap = { pending:["Pendiente","b-warn"], processing:["En proceso","b-cy"], shipped:["Enviado","b-warn"], delivered:["Entregado","b-ok"], cancelled:["Cancelado","b-off"] };
...
const COLOR_MAP = c => c==="Blanco"?"#fff":c==="Negro"?"#111":c==="Rojo"?"#ef4444":c==="Azul"?"#3b82f6":c==="Verde"?"#22c55e":c==="Rosa"?"#ec4899":c==="Gris"?"#6b7280":c==="Beige"?"#d4b89a":c==="Amarillo"?"#eab308":c==="Café"?"#92400e":c==="Kaki"?"#a3a068":c==="Terracota"?"#c2553c":c==="Coral"?"#ff6b6b":"var(--bg4)";
      ...
      <div className="chart-bar-wrap" style={{height}}>
        {data.map((v,i)=>(
          <div key={i} className="chart-bar"
            style={{height:`${(v/max*100)}%`,background:color,opacity:.7+(i===data.length-1?.3:0)}}
            title={`${MONTHS[i]}: ${v}`}/>
        ))}
      </div>
  ...
  return (
    <div className="login-pg">
      ...
      <div className="login-card">
        ...
        <div className="login-sub">Red de moda y distribución — Medellín · 400+ tiendas</div>
        ...
        <div className="fg">
          ...
          <input type="email" placeholder="correo@ejemplo.com" value={email} onChange={e=>setEmail(e.target.value)} onKeyPress={kp}/>
        </div>
...
/* ─── CATALOG (CLIENT) ─── */
function Catalog({ user, db, setDb }) {
  ...
  const placeOrder = () => {
    ...
    const order = {
      id:genId(),storeId,storeName:db.stores.find(s=>s.id===storeId)?.name||"",
      userId:user.id,items:cart.map(x=>({productId:x.id,name:x.name,qty:x.qty,price:x.price,color:x.chosenColor,size:x.chosenSize})),
      total:cartTotal,status:"pending",createdAt:new Date().toISOString().slice(0,10)
    };
    ...
    setDb(newDb); setCart([]); setOrdered(true); setShowCart(false);
  ...
  return (
    <div>
      {ordered && <div className="alert al-ok">✅ ¡Pedido realizado! Puedes seguirlo en "Mis Pedidos". El equipo de la tienda te contactará pronto.</div>}

      <div className="sec-hd">
        <div>
          <div className="sec-ttl">👗 Catálogo de Moda</div>
          <div style={{fontSize:".75rem",color:"var(--t2)",marginTop:2}}>{prods.length} prendas disponibles · {activeStores.length} tiendas activas</div>
        </div>
        <button className="btn btn-cy btn-sm" onClick={()=>setShowCart(true)}>
      ...
      <div className="filters">
        <input placeholder="🔍 Buscar prendas, marcas..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:2,minWidth:180}}/>
      ...
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
        {["Vestidos","Jeans","Hoodies","Blazers","Calzado","Accesorios","Deportivo","Camisas"].map(c=>(
              ...
              return (
                <div key={p.id} className="prod-card" onClick={()=>{setSel(p);setSelColor(p.colors?.[0]||"");setSelSize(p.sizes?.[0]||"")}}>
                  <div className="prod-thumb">
                    {p.isNew && <span className="prod-badge"><span className="badge b-cy" style={{fontSize:".58rem"}}>NUEVO</span></span>}
                    {p.discount>0 && <span className="prod-badge-sale"><span className="badge" style={{background:"var(--re10)",color:"var(--re)",fontSize:".58rem"}}>-{p.discount}%</span></span>}
        ...
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setSel(null)}>
          <div className="modal modal-lg">
            <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
              <div style={{flex:"0 0 180px",height:200,background:"linear-gradient(135deg,var(--bg3),var(--bg4))",borderRadius:"var(--r)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"6rem",border:"1px solid var(--bd)"}}>
                {sel.emoji}
              </div>
              <div style={{flex:1,minWidth:200}}>
                <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
                  ...
                  <span className="pill" style={{color:sel.stock<=sel.minStock?"var(--re)":sel.stock>30?"var(--gr)":"var(--ye)"}}>
                    Stock: {sel.stock}
                  </span>
                </div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
                  ...
                  {sel.discount>0 && <div style={{textDecoration:"line-through",color:"var(--t2)",fontSize:"1rem"}}>{fmt(sel.price)}</div>}
                  {sel.discount>0 && <span className="badge" style={{background:"var(--re10)",color:"var(--re)"}}>-{sel.discount}%</span>}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
                  <Stars value={sel.rating}/>
                  <span style={{color:"var(--ye)",fontFamily:"var(--mono)",fontWeight:700}}>{sel.rating}</span>
                  <span style={{color:"var(--t2)",fontSize:".78rem"}}>/5 · {sel.sales} ventas</span>
                </div>
              </div>
            </div>

            {sel.colors && sel.colors.length > 0 && (
              <div style={{marginBottom:14}}>
                <label>COLOR: {selColor}</label>
                <div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap"}}>
                  {sel.colors.map(c=>(
                    <div key={c} onClick={()=>setSelColor(c)}
                      className={`color-swatch${selColor===c?" sel":""}`}
                      style={{width:28,height:28,background:COLOR_MAP(c),border:"1px solid var(--bd)"}}
                      title={c}/>
                  ))}
                </div>
              </div>
            ...
            {sel.sizes && sel.sizes.length > 0 && sel.sizes[0]!=="Única" && (
              <div style={{marginBottom:16}}>
                <label>TALLA: {selSize}</label>
                <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                  {sel.sizes.map(s=>(
                    <button key={s} className={`size-btn${selSize===s?" sel":""}`} onClick={()=>setSelSize(s)}>{s}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div style={{display:"flex",gap:8,marginTop:14,flexWrap:"wrap"}}>
              ...
              <button className="btn btn-cy" onClick={()=>{addCart({...sel,chosenColor:selColor,chosenSize:selSize});setSel(null)}} disabled={sel.stock===0}>🛒 {sel.stock===0?"Agotado":"Agregar al carrito"}</button>
      ...
      {showCart && (
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setShowCart(false)}>
          <div className="modal">
            ...
            {cart.length===0
              ? <div className="empty"><div className="empty-ico">🛒</div><div>El carrito está vacío</div></div>
              : <>
                  {cart.map(x=>(
                    <div key={x.id} style={{display:"flex",alignItems:"center",gap:12,marginBottom:10,padding:"10px 0",borderBottom:"1px solid var(--bd)"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        {(x.chosenColor||x.chosenSize) && <div style={{fontSize:".7rem",color:"var(--t1)",marginTop:2}}>
                          {x.chosenColor && `🎨 ${x.chosenColor}`} {x.chosenSize && `📐 ${x.chosenSize}`}
                        </div>}
                      </div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",fontFamily:"var(--cond)",fontSize:"1.15rem",fontWeight:800,margin:"16px 0 8px",padding:"12px 0",borderTop:"1px solid var(--bd)"}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button className="btn btn-sec" onClick={()=>setShowCart(false)}>Cancelar</button>
                  </div>
                </>
            }
          </div>
    ...
    <div>
      <div className="sec-hd">
        <div>
          ...
          <div style={{fontSize:".75rem",color:"var(--t2)",marginTop:2}}>
            {userLoc?<span className="text-gr">📍 GPS activo · ordenado por distancia</span>:<span>Ubícate para ver tiendas cercanas</span>}
          </div>
        </div>
        {userLoc && <span className="badge b-ok">📍 Ubicado</span>}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:16,alignItems:"start"}}>
        <div className="map-wrap" style={{height:520,position:"relative"}}>
          ...
          <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none"}} viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <text x="35%" y="42%" fill="#253c5a" fontSize="5" fontFamily="DM Mono">Autopista Norte</text>
            <text x="8%" y="65%" fill="#253c5a" fontSize="5" fontFamily="DM Mono">Av. Regional</text>
            <text x="48%" y="82%" fill="#253c5a" fontSize="5" fontFamily="DM Mono">Av. El Poblado</text>
          </svg>

          <div style={{position:"absolute",top:8,left:12,color:"var(--t2)",fontSize:".58rem",fontFamily:"var(--mono)",letterSpacing:".1em",zIndex:5}}>
          ...
          {stores.map(s=>{
            ...
            const isFiltered = visibleStores.every(vs=>vs.id!==s.id)&&s.id!==sel;
            const dist = userLoc?haversine(userLoc.lat,userLoc.lng,s.lat,s.lng):null;
            const open = isStoreOpen(s);
            return (
              <div key={s.id} style={{position:"absolute",left:x,top:y,transform:"translate(-50%,-50%)",zIndex:isSel?10:5,cursor:"pointer",opacity:isFiltered?.15:1,transition:"opacity .2s"}}
                ...
                <div style={{position:"relative"}}>
                  ...
                  <div style={{background:isSel?"var(--cy)":s.active?"var(--bg3)":"rgba(255,51,85,.12)",border:`2px solid ${isSel?"var(--cy)":s.active?"var(--bd2)":"var(--re)"}`,borderRadius:"50%",width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.1rem",boxShadow:isSel?`0 0 20px var(--cy),0 0 40px rgba(0,229,255,.3)`:s.active?"0 2px 8px rgba(0,0,0,.5)":"none",transition:"all .2s",position:"relative",zIndex:1}}>
                    {s.emoji}
                    {s.active && open && <div style={{position:"absolute",top:-2,right:-2,width:9,height:9,borderRadius:"50%",background:"var(--gr)",border:"1px solid var(--bg)"}}/>}
                  </div>
                  {isSel && selStore && (
                    <div style={{position:"absolute",bottom:"calc(100% + 12px)",left:"50%",transform:"translateX(-50%)",background:"var(--bg1)",border:`1px solid ${selStore.theme||"var(--cy)"}`,borderRadius:12,padding:"12px 16px",minWidth:220,whiteSpace:"nowrap",zIndex:20,boxShadow:"0 8px 32px rgba(0,0,0,.7)"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                        <span style={{fontSize:"1.3rem"}}>{selStore.emoji}</span>
                        <div>
                          <div style={{fontFamily:"var(--cond)",fontWeight:800,fontSize:".95rem",color:selStore.theme||"var(--cy)"}}>{selStore.name}</div>
                          <div style={{fontSize:".65rem",color:"var(--t2)"}}>{selStore.address}</div>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:8,fontSize:".7rem",color:"var(--t1)",marginBottom:6,flexWrap:"wrap"}}>
                        ...
                        <span className={`badge ${open?"b-ok":"b-off"}`} style={{fontSize:".58rem"}}>{open?"ABIERTO":"CERRADO"}</span>
                      </div>
                      <div style={{fontFamily:"var(--mono)",fontSize:".65rem",color:"var(--t2)",marginBottom:8}}>🌐 {selStore.subdomain}.distrimed.co</div>
                      {selProducts.length>0 && (
                        <div style={{borderTop:"1px solid var(--bd)",paddingTop:6}}>
                          <div style={{fontSize:".6rem",color:"var(--t2)",marginBottom:4,fontFamily:"var(--cond)",fontWeight:700}}>PRODUCTOS DESTACADOS</div>
                          {selProducts.map(p=>(
                            <div key={p.id} style={{fontSize:".7rem",color:"var(--t1)",padding:"1px 0"}}>{p.emoji} {p.name} · <span style={{color:"var(--cy)",fontFamily:"var(--mono)"}}>{fmt(p.price)}</span></div>
                          ))}
                        </div>
                      )}
                    </div>
          ...
          {userLoc && (
            <div style={{position:"absolute",left:toX(userLoc.lng),top:toY(userLoc.lat),transform:"translate(-50%,-50%)",zIndex:15}}>
              <div style={{width:16,height:16,borderRadius:"50%",background:"var(--or)",boxShadow:"0 0 20px var(--or)",border:"2px solid #fff"}}/>
              <div style={{position:"absolute",inset:-8,borderRadius:"50%",border:"2px solid var(--or)",animation:"ring 2s infinite"}}/>
              <div style={{position:"absolute",bottom:"calc(100% + 4px)",left:"50%",transform:"translateX(-50%)",background:"var(--bg1)",border:"1px solid var(--or)",borderRadius:6,padding:"3px 8px",fontSize:".6rem",color:"var(--or)",fontFamily:"var(--mono)",whiteSpace:"nowrap",zIndex:10}}>
                TÚ ESTÁS AQUÍ
              </div>
            </div>
          )}

          <div style={{position:"absolute",bottom:10,left:10,background:"rgba(5,12,26,.93)",border:"1px solid var(--bd)",borderRadius:8,padding:"8px 12px",fontSize:".65rem",color:"var(--t1)",zIndex:5}}>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              <span style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:10,height:10,borderRadius:"50%",background:"var(--bg3)",border:"1px solid var(--bd2)"}}/>Tienda activa</span>
              <span style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:10,height:10,borderRadius:"50%",background:"rgba(255,51,85,.15)",border:"1px solid var(--re)"}}/>Inactiva</span>
              {userLoc && <span style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:10,height:10,borderRadius:"50%",background:"var(--or)"}}/>Tu ubicación</span>}
              <span style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:8,height:8,borderRadius:"50%",background:"var(--gr)"}}/>Abierta ahora</span>
            </div>
          </div>
          <div style={{position:"absolute",bottom:10,right:10,background:"rgba(5,12,26,.93)",border:"1px solid var(--bd)",borderRadius:8,padding:"6px 10px",fontSize:".65rem",color:"var(--cy)",fontFamily:"var(--mono)",zIndex:5}}>
            {active.length}/{stores.length} tiendas · {visibleStores.length} visibles
          </div>
        </div>

        <div style={{height:520,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
          <div style={{fontSize:".7rem",color:"var(--t2)",fontFamily:"var(--cond)",fontWeight:700,textTransform:"uppercase",letterSpacing:".08em",padding:"0 4px 6px",borderBottom:"1px solid var(--bd)",marginBottom:4}}>
            {userLoc?"📍 Por distancia":"Tiendas disponibles"}
          </div>
          {visibleStores
            .map(s=>({...s,dist:userLoc?haversine(userLoc.lat,userLoc.lng,s.lat,s.lng):null}))
            .sort((a,b)=>a.dist!==null&&b.dist!==null?a.dist-b.dist:b.totalSales-a.totalSales)
            .map(s=>{
              const open = isStoreOpen(s);
              return (
                <div key={s.id} className={`store-card${sel===s.id?" sel":""}`} onClick={()=>setSel(sel===s.id?null:s.id)}>
                  <div className="s-emoji" style={{position:"relative"}}>
                    {s.emoji}
                    {s.active && open && <div style={{position:"absolute",top:0,right:0,width:8,height:8,borderRadius:"50%",background:"var(--gr)",border:"1px solid var(--bg)"}}/>}
                  </div>
                  <div className="s-info">
                    <div className="s-name" style={{color:s.active?"var(--t0)":"var(--t2)"}}>{s.name}</div>
                    <div className="s-addr">{s.address}</div>
                    <div style={{display:"flex",gap:6,marginTop:3,fontSize:".65rem",color:"var(--t2)",flexWrap:"wrap"}}>
                      <span>⭐ {s.rating}</span>
                      <span>👗 {db.products.filter(p=>p.storeId===s.id).length}</span>
                      <span className="pill" style={{fontSize:".58rem"}}>{s.category}</span>
                      <span className={`badge ${s.active?"b-ok":"b-off"}`} style={{fontSize:".58rem"}}>{s.active?"ON":"OFF"}</span>
                    </div>
                    {s.subdomain && <div style={{marginTop:3,fontFamily:"var(--mono)",fontSize:".62rem",color:"var(--pu)"}}>{s.subdomain}.distrimed.co</div>}
                  </div>
                  {s.dist!==null && <div className="s-dist">{s.dist}km</div>}
                </div>
              );
            })
          }
  ...
  return (
    <div>
      <div className="sec-hd">
        ...
        <span className="badge b-cy">{orders.length} pedidos</span>
      </div>
      {orders.length===0
        ? <div className="empty"><div className="empty-ico">📋</div><div className="empty-ttl">Sin pedidos aún</div><div>Agrega prendas al carrito para comenzar</div></div>
        : orders.map(o=>{
          const store = db.stores.find(s=>s.id===o.storeId);
          return (
            <div key={o.id} style={{background:"var(--bg2)",border:"1px solid var(--bd)",borderRadius:"var(--rl)",padding:20,marginBottom:12}}>
              ...
              {o.status!=="cancelled" && (
                <div className="order-status-bar" style={{marginBottom:12}}>
                  {steps.map((s,i)=><div key={s} className={`os-step${steps.indexOf(o.status)>=i?" done":""}`}/>)}
                </div>
              )}
              {o.items.map((it,i)=>(
...
function StoreAdmin({ user, db, setDb }) {
  ...
  if(!store) return <div className="empty"><div className="empty-ico">❌</div><div className="empty-ttl">Tienda no encontrada</div></div>;
  ...
  const lowStock = myProducts.filter(p=>p.stock<=p.minStock&&p.stock>0);
  ...
  const save = () => {
    if(!form.name||!form.price) return showAlert("err","Nombre y precio son requeridos");
    ...
    if(editing) {
      ...
      showAlert("ok","✅ Producto actualizado");
    } else {
      const np = {id:genId(),storeId:store.id,...data,rating:0,sales:0};
      setDb({...db,products:[...db.products,np]});
      showAlert("ok","✅ Producto creado");
    }
    setShowModal(false);
  };

  const del = id => { if(!confirm("¿Eliminar este producto?")) return; setDb({...db,products:db.products.filter(p=>p.id!==id)}); showAlert("ok","Producto eliminado"); };
  const stockChange = (id,val) => setDb({...db,products:db.products.map(p=>p.id===id?{...p,stock:Math.max(0,p.stock+val)}:p)});
  const updateOrderStatus = (oid,status) => setDb({...db,orders:db.orders.map(o=>o.id===oid?{...o,status}:o)});
  const saveHours = () => { setDb({...db,stores:db.stores.map(s=>s.id===store.id?{...s,hours:hoursForm}:s)}); setHoursForm(null); showAlert("ok","✅ Horarios actualizados"); };
  const saveProfile = () => { setDb({...db,stores:db.stores.map(s=>s.id===store.id?{...s,...profileForm,socialMedia:{instagram:profileForm.instagram,whatsapp:profileForm.whatsapp}}:s)}); setProfileForm(null); showAlert("ok","✅ Perfil actualizado"); };

  return (
    <div>
      {alert && <div className={`alert ${alert.type==="ok"?"al-ok":"al-err"}`}>{alert.msg}</div>}
      ...
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20,padding:16,background:"var(--bg2)",borderRadius:"var(--rl)",border:"1px solid var(--bd)",flexWrap:"wrap"}}>
        ...
        <div style={{flex:1,minWidth:200}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}>
            ...
            <span className={`badge ${isOpen?"b-ok":"b-off"}`}>{isOpen?"🟢 ABIERTA AHORA":"🔴 CERRADA"}</span>
      ...
      <div className="stats-grid">
        ...
        <div className="stat"><div className="stat-ico">💰</div><div className="stat-val text-cy">{fmtK(store.totalSales)}</div><div className="stat-lbl">Ventas totales</div><div className="stat-sub">{fmtK(revenue)} este período</div></div>
        <div className="stat"><div className="stat-ico">⭐</div><div className="stat-val text-ye">{avgRating}</div><div className="stat-lbl">Rating promedio</div><div className="stat-sub">{store.ordersCount} pedidos totales</div></div>
      </div>
      ...
      <div className="tabs">
        {[["dashboard","📊 Dashboard"],["products","👗 Productos"],["orders","📋 Pedidos"],["hours","🕐 Horarios"],["profile","🏪 Perfil"],["page","🌐 Mi Página"]].map(([k,l])=>(
        ...
        <div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
            ...
            <div className="card">
              ...
              {myProducts.sort((a,b)=>b.sales-a.sales).slice(0,4).map(p=>(
            ...
            {myOrders.slice(0,4).map(o=>(
              <div key={o.id} className="feed-item">
                ...
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{fontFamily:"var(--mono)",color:"var(--cy)"}}>{fmt(o.total)}</span>
                  <span className={`badge ${statusMap[o.status]?.[1]}`}>{statusMap[o.status]?.[0]}</span>
                </div>
              </div>
            ))}
          </div>
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>
                        ...
                        <span className={`fw8 ${p.stock===0?"text-re":p.stock<=p.minStock?"text-ye":"text-gr"}`} style={{minWidth:28,textAlign:"center"}}>{p.stock}</span>
                        <button className="btn btn-sec btn-xs" onClick={()=>stockChange(p.id,1)}>+</button>
                        <button className="btn btn-sec btn-xs" onClick={()=>stockChange(p.id,10)}>+10</button>
                      </div>
                    </td>
                    <td style={{fontSize:".72rem",color:"var(--t1)"}}>{(p.sizes||[]).join(", ")||"—"}</td>
                    <td><span style={{color:"var(--t1)"}}>{p.sales}</span></td>
                    <td><Stars value={p.rating}/><span style={{fontSize:".65rem",color:"var(--ye)",marginLeft:3}}>{p.rating}</span></td>
                    <td>{p.stock===0?<span className="badge b-off">AGOTADO</span>:p.stock<=p.minStock?<span className="badge b-warn">BAJO</span>:<span className="badge b-ok">OK</span>}</td>
                    <td>
      ...
      {tab==="orders" && (
        <>
          <div className="sec-ttl mb-3">Pedidos Recibidos ({myOrders.length})</div>
          {myOrders.length===0
            ? <div className="empty"><div className="empty-ico">📋</div><div>Sin pedidos aún</div></div>
            : <div className="tbl-wrap"><table>
                <thead><tr><th>ID</th><th>Fecha</th><th>Prendas</th><th>Total</th><th>Estado</th><th>Acción</th></tr></thead>
                <tbody>
                  {myOrders.map(o=>(
                    <tr key={o.id}>
                      <td><span className="mono text-xs text-muted">#{o.id.toUpperCase().slice(0,6)}</span></td>
                      <td style={{fontSize:".8rem"}}>{o.createdAt}</td>
                      <td style={{fontSize:".78rem",color:"var(--t1)"}}>{o.items.map(i=>`${i.qty}x ${i.name}`).join(", ")}</td>
                      <td><span className="mono text-cy">{fmt(o.total)}</span></td>
                      <td><span className={`badge ${statusMap[o.status]?.[1]||"b-cy"}`}>{statusMap[o.status]?.[0]||o.status}</span></td>
                      <td>
                        <select value={o.status} onChange={e=>updateOrderStatus(o.id,e.target.value)} style={{padding:"4px 8px",fontSize:".72rem",width:"auto"}}>
      ...
      {tab==="hours" && (
        <div style={{maxWidth:600}}>
          ...
          <div className="store-hours-grid">
            {DAYS.map((d,i)=>{
              const hrs = store.hours?.[d];
              return (
                <div key={d} className="hours-chip">
                  <div className="hours-day">{DAYS_LABEL[i]}</div>
                  {hrs && hrs!=="Cerrado" ? <div className="hours-time">{hrs}</div> : <div className="hours-closed">Cerrado</div>}
                </div>
              );
            })}
          </div>
          <div style={{marginTop:14,padding:12,background:"var(--bg3)",borderRadius:"var(--r)",fontSize:".78rem",color:"var(--t2)"}}>
            Estado actual: {isOpen?<span className="text-gr">🟢 Abierta ahora</span>:<span className="text-re">🔴 Cerrada</span>}
          </div>
          {hoursForm && (
            <div className="overlay" onClick={e=>e.target===e.currentTarget&&setHoursForm(null)}>
              <div className="modal">
                <div className="modal-ttl">🕐 Editar Horarios</div>
                <div style={{fontSize:".78rem",color:"var(--t2)",marginBottom:14}}>Formato: 9:00–18:00 o "Cerrado"</div>
                {DAYS.map((d,i)=>(
                  <div key={d} className="fg">
                    <label>{DAYS_LABEL[i]}</label>
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <input value={hoursForm[d]||""} onChange={e=>setHoursForm({...hoursForm,[d]:e.target.value})} placeholder="9:00–18:00"/>
                      <button className="btn btn-sec btn-xs" onClick={()=>setHoursForm({...hoursForm,[d]:"Cerrado"})}>Cerrado</button>
                    </div>
                  </div>
                ))}
      ...
      {tab==="profile" && (
        <div style={{maxWidth:600}}>
          <div className="sec-hd">
            ...
            <button className="btn btn-cy btn-sm" onClick={()=>setProfileForm({name:store.name,address:store.address,phone:store.phone,description:store.description,emoji:store.emoji,category:store.category,tags:(store.tags||[]).join(","),theme:store.theme||"#00e5ff",instagram:store.socialMedia?.instagram||"",whatsapp:store.socialMedia?.whatsapp||""})}>✏️ Editar</button>
          </div>
          <div className="card">
            <div style={{display:"flex",gap:16,marginBottom:16}}>
              <span style={{fontSize:"3.5rem"}}>{store.emoji}</span>
              <div>
                <div style={{fontFamily:"var(--cond)",fontSize:"1.3rem",fontWeight:800}}>{store.name}</div>
                <div style={{fontSize:".8rem",color:"var(--t2)",marginTop:4}}>{store.description}</div>
              </div>
            </div>
            <div className="fg-row">
              <div><label>Dirección</label><p style={{fontSize:".875rem",color:"var(--t1)"}}>{store.address}</p></div>
              <div><label>Teléfono</label><p style={{fontSize:".875rem",color:"var(--t1)"}}>{store.phone}</p></div>
            </div>
            <div style={{marginTop:12,display:"flex",gap:12}}>
              {store.socialMedia?.instagram && <span style={{fontSize:".8rem",color:"var(--t1)"}}>📸 {store.socialMedia.instagram}</span>}
              {store.socialMedia?.whatsapp && <a href={`https://wa.me/57${store.socialMedia.whatsapp}`} target="_blank" rel="noreferrer" style={{fontSize:".8rem",color:"var(--gr)"}}>💬 WhatsApp</a>}
            </div>
          </div>
          {profileForm && (
            <div className="overlay" onClick={e=>e.target===e.currentTarget&&setProfileForm(null)}>
              <div className="modal modal-lg">
                ...
                <div className="fg"><label>Descripción</label><textarea value={profileForm.description} onChange={e=>setProfileForm({...profileForm,description:e.target.value})} rows={2}/></div>
                <div className="fg-row">
                  <div className="fg"><label>Emoji / Logo</label><input value={profileForm.emoji} onChange={e=>setProfileForm({...profileForm,emoji:e.target.value})} style={{fontSize:"1.5rem",textAlign:"center"}}/></div>
                  <div className="fg"><label>Color de marca</label><input type="color" value={profileForm.theme} onChange={e=>setProfileForm({...profileForm,theme:e.target.value})} style={{height:42,padding:4,cursor:"pointer"}}/></div>
                </div>
                <div className="fg-row">
                  <div className="fg"><label>Instagram</label><input value={profileForm.instagram} onChange={e=>setProfileForm({...profileForm,instagram:e.target.value})} placeholder="@mitienda"/></div>
                  <div className="fg"><label>WhatsApp (sin +57)</label><input value={profileForm.whatsapp} onChange={e=>setProfileForm({...profileForm,whatsapp:e.target.value})} placeholder="3001234567"/></div>
                </div>
                <div style={{display:"flex",gap:8,marginTop:8}}>
                  <button className="btn btn-cy" onClick={saveProfile}>💾 Guardar perfil</button>
      ...
      {tab==="page" && (
        <div>
          <div className="sec-hd">
            <div>
              <div className="sec-ttl">🌐 Mi Página Web</div>
              <div style={{fontSize:".75rem",color:"var(--t2)",marginTop:2}}>Vista previa de cómo ven tu tienda los clientes</div>
            </div>
            <div className="subdomain-pill">🌐 {store.subdomain}.distrimed.co</div>
          </div>
          <div style={{background:"var(--bg1)",border:"1px solid var(--bd)",borderRadius:"var(--rxl)",overflow:"hidden"}}>
            <div className="store-hero-bg" style={{background:`linear-gradient(135deg, ${store.theme||"#00e5ff"}18 0%, var(--bg2) 100%)`,borderBottom:"1px solid var(--bd)"}}>
              <div style={{textAlign:"center",position:"relative",zIndex:1}}>
                <div style={{fontSize:"5rem",marginBottom:8}}>{store.emoji}</div>
                <div style={{fontFamily:"var(--cond)",fontSize:"2rem",fontWeight:900,color:store.theme||"var(--cy)"}}>{store.name}</div>
                <div style={{fontSize:".9rem",color:"var(--t2)",marginTop:4}}>{store.description}</div>
                <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:12,flexWrap:"wrap"}}>
                  {store.tags?.map(t=><span key={t} className="filter-chip" style={{cursor:"default"}}>{t}</span>)}
                </div>
              </div>
            </div>
            <div style={{padding:24}}>
              <div style={{marginBottom:20}}>
                <div style={{fontFamily:"var(--cond)",fontWeight:700,marginBottom:10,color:"var(--t1)"}}>HORARIOS</div>
                <div className="store-hours-grid">
                  {DAYS.map((d,i)=>{
                    const hrs = store.hours?.[d];
                    return (
              ...
              <div>
                <div style={{fontFamily:"var(--cond)",fontWeight:700,marginBottom:10,color:"var(--t1)"}}>NUESTRAS PRENDAS</div>
                <div className="prod-grid" style={{gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))"}}>
                  {myProducts.slice(0,6).map(p=>(
                    <div key={p.id} className="prod-card" style={{cursor:"default"}}>
                      <div className="prod-thumb" style={{height:100}}>{p.emoji}</div>
                      <div className="prod-body">
                        <div className="prod-name" style={{fontSize:".85rem"}}>{p.name}</div>
                        <div className="prod-price" style={{fontSize:".85rem"}}>{fmt(p.price)}</div>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{marginTop:20,padding:14,background:"var(--bg3)",borderRadius:"var(--r)",display:"flex",gap:16,alignItems:"center",flexWrap:"wrap"}}>
                <span>📍 {store.address}</span>
                <span>📞 {store.phone}</span>
                {store.socialMedia?.instagram && <span>📸 {store.socialMedia.instagram}</span>}
                {store.socialMedia?.whatsapp && <a href={`https://wa.me/57${store.socialMedia.whatsapp}`} target="_blank" rel="noreferrer" style={{color:"var(--gr)"}}>💬 WhatsApp</a>}
              </div>
            </div>
          </div>
        </div>
      ...
      {showModal && (
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div className="modal modal-lg">
            ...
            <div className="fg"><label>Nombre *</label><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Nombre de la prenda"/></div>
            <div className="fg-row">
              <div className="fg"><label>Precio COP *</label><input type="number" value={form.price} onChange={e=>setForm({...form,price:e.target.value})} placeholder="0"/></div>
              <div className="fg"><label>Stock actual</label><input type="number" value={form.stock} onChange={e=>setForm({...form,stock:e.target.value})} placeholder="0"/></div>
            </div>
            <div className="fg-row">
              ...
              <div className="fg"><label>Emoji</label><input value={form.emoji} onChange={e=>setForm({...form,emoji:e.target.value})} placeholder="👗" style={{fontSize:"1.5rem",textAlign:"center"}}/></div>
            </div>
            <div className="fg-row">
              <div className="fg"><label>Marca / Brand</label><input value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})} placeholder="Zara, Nike, etc."/></div>
            ...
            <div className="fg-row">
              <div className="fg"><label>Descuento (%)</label><input type="number" value={form.discount} onChange={e=>setForm({...form,discount:e.target.value})} placeholder="0" min="0" max="90"/></div>
              <div className="fg" style={{justifyContent:"flex-end",paddingBottom:4}}>
                ...
                <label className="toggle-wrap" style={{cursor:"pointer",marginTop:8}}>
                  ...
                  <span style={{fontSize:".85rem"}}>{form.isNew?"✓ Es producto nuevo":"No es nuevo"}</span>
                </label>
              </div>
            </div>
            <div className="fg"><label>Descripción</label><textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})} rows={2} placeholder="Descripción breve de la prenda..."/></div>
            <div style={{display:"flex",gap:8,marginTop:8}}>
...
function SuperAdmin({ db, setDb }) {
  ...
  const toggleStore = id => { setDb({...db,stores:db.stores.map(s=>s.id===id?{...s,active:!s.active}:s)}); showAlert("ok","Estado de tienda actualizado"); };
  const toggleUser = id => { setDb({...db,users:db.users.map(u=>u.id===id?{...u,active:!u.active}:u)}); showAlert("ok","Estado de usuario actualizado"); };

  const openNewStore = () => {
    setEditingStore(null);
    setStoreForm({name:"",address:"",phone:"",lat:"6.2442",lng:"-75.5812",category:"Ropa Mujer",emoji:"🏪",description:"",subdomain:"",theme:"#00e5ff",templateId:""});
    setShowStoreModal(true);
  };

  const openEditStore = s => {
    setEditingStore(s);
    setStoreForm({name:s.name,address:s.address,phone:s.phone,lat:String(s.lat),lng:String(s.lng),category:s.category,emoji:s.emoji,description:s.description,subdomain:s.subdomain||"",theme:s.theme||"#00e5ff",templateId:""});
    setShowStoreModal(true);
  };
  ...
  const saveStore = () => {
    if(!storeForm.name||!storeForm.address) return showAlert("err","Nombre y dirección requeridos");
    if(editingStore) {
      setDb({...db,stores:db.stores.map(s=>s.id===editingStore.id?{...s,...storeForm,lat:+storeForm.lat,lng:+storeForm.lng}:s)});
      showAlert("ok","✅ Tienda actualizada");
    } else {
      ...
      showAlert("ok","✅ Tienda creada exitosamente");
    }
    setShowStoreModal(false);
  };

  const deleteStore = id => { if(!confirm("¿Eliminar esta tienda permanentemente?")) return; setDb({...db,stores:db.stores.filter(s=>s.id!==id),products:db.products.filter(p=>p.storeId!==id)}); showAlert("ok","Tienda eliminada"); };
  ...
  return (
    <div>
      ...
      <div className="tabs">
        {[["dashboard","⚡ Dashboard"],["stores","🏪 Tiendas"],["products","👗 Catálogo"],["users","👥 Usuarios"],["templates","🎨 Templates"]].map(([k,l])=>(
          <button key={k} className={`tab${tab===k?" on":""}`} onClick={()=>setTab(k)}>{l}</button>
        ))}
      </div>
      ...
      {tab==="dashboard" && (
        <div>
          <div className="stats-grid">
            ...
            <div className="stat"><div className="stat-ico">👥</div><div className="stat-val">{db.users.length}</div><div className="stat-lbl">Usuarios</div><div className="stat-sub">{clients} clientes</div></div>
            <div className="stat"><div className="stat-ico">📊</div><div className="stat-val text-or">{db.orders.length>0?fmtK(totalRev/db.orders.length):"$0"}</div><div className="stat-lbl">Ticket promedio</div><div className="stat-sub">por pedido</div></div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
            <div className="card">
              <div style={{fontFamily:"var(--cond)",fontSize:".8rem",fontWeight:700,color:"var(--t2)",marginBottom:10}}>VENTAS 12 MESES</div>
              <MiniChart data={db.salesByMonth} color="var(--cy)"/>
            </div>
            <div className="card">
      ...
      {tab==="stores" && (
        <div>
          <div className="sec-hd">
            <div>
              <div className="sec-ttl">🏪 Gestión de Tiendas ({db.stores.length})</div>
              <div style={{fontSize:".75rem",color:"var(--t2)",marginTop:2}}>{activeStores} activas · {db.stores.length-activeStores} inactivas</div>
            </div>
          ...
          <div className="tbl-wrap">
            <table>
              <thead><tr><th>Tienda</th><th>Categoría</th><th>Subdominio</th><th>Productos</th><th>Ventas</th><th>Rating</th><th>Estado</th><th>Horario</th><th>Acciones</th></tr></thead>
              <tbody>
                    ...
                    <tr key={s.id}>
                      <td>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          ...
                          <div>
                            <div style={{fontFamily:"var(--cond)",fontWeight:700}}>{s.name}</div>
                            <div style={{fontSize:".7rem",color:"var(--t2)"}}>{s.address}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td>
                        <div style={{display:"flex",gap:4}}>
                          <button className="btn btn-sec btn-xs" onClick={()=>openEditStore(s)}>✏️</button>
                          <button className="btn btn-re btn-xs" onClick={()=>deleteStore(s.id)}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      {tab==="products" && (
        <div>
          <div className="sec-hd">
            <div className="sec-ttl">👗 Catálogo Global ({db.products.length} prendas)</div>
          </div>
          <div className="tbl-wrap">
            <table>
                ...
                {db.products.map(p=>{
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td style={{fontSize:".78rem"}}>{store?.emoji} {store?.name||"—"}</td>
                      <td><span className="mono text-cy">{fmt(p.price)}</span></td>
                      <td><span className={p.stock===0?"text-re":p.stock<=(p.minStock||10)?"text-ye":"text-gr"} style={{fontFamily:"var(--mono)",fontWeight:700}}>{p.stock}</span></td>
                      <td><span style={{color:"var(--t1)"}}>{p.sales}</span></td>
                      <td><span style={{color:"var(--ye)",fontFamily:"var(--mono)"}}>{p.rating} ⭐</span></td>
                      <td>{p.isNew&&<span className="badge b-cy" style={{fontSize:".55rem"}}>NUEVO</span>}{p.discount>0&&<span className="badge b-or" style={{fontSize:".55rem",marginLeft:2}}>-{p.discount}%</span>}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {tab==="users" && (
        <div>
          <div className="sec-hd">
            <div className="sec-ttl">👥 Gestión de Usuarios ({db.users.length})</div>
          </div>
          <div className="tbl-wrap">
            <table>
                ...
                {db.users.map(u=>{
                  ...
                  return (
                    <tr key={u.id}>
                      <td>
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          <div className={`av ${u.role==="superadmin"?"av-cy":u.role==="store"?"av-or":"av-gr"}`}>{u.name.slice(0,2).toUpperCase()}</div>
                          <span style={{fontFamily:"var(--cond)",fontWeight:600}}>{u.name}</span>
                        </div>
                      </td>
      ...
      {tab==="templates" && (
        <div>
          <div className="sec-hd">
            <div>
              <div className="sec-ttl">🎨 Templates de Tiendas</div>
              <div style={{fontSize:".75rem",color:"var(--t2)",marginTop:2}}>Plantillas predefinidas para nuevas tiendas</div>
            </div>
          </div>
            ...
            {db.templates.map(t=>(
              <div key={t.id} className="card" style={{borderColor:t.primaryColor+"44",cursor:"pointer"}} onClick={()=>{applyTemplate(t);setTab("stores");openNewStore()}}>
                ...
                <div style={{marginTop:12}}>
                  <button className="btn btn-sec btn-sm w-full" onClick={e=>{e.stopPropagation();applyTemplate(t)}}>✓ Usar template</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="card" style={{maxWidth:480}}>
            <div style={{fontFamily:"var(--cond)",fontWeight:800,marginBottom:12}}>⚙️ Configuración de red</div>
            <div className="fg"><label>Dominio base</label><input defaultValue="distrimed.co" readOnly style={{opacity:.6}}/></div>
            <div className="fg"><label>Total de subdominios activos</label><input value={db.stores.filter(s=>s.active&&s.subdomain).length} readOnly style={{opacity:.6}}/></div>
            <div style={{marginTop:8,padding:10,background:"var(--bg3)",borderRadius:"var(--r)",fontSize:".78rem",color:"var(--t2)"}}>
              ℹ️ Cada tienda activa tiene su subdominio en <span style={{color:"var(--pu)"}}>[subdomain].distrimed.co</span>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal modal-lg">
            ...
            {showTemplates && (
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14,padding:12,background:"var(--bg3)",borderRadius:"var(--r)"}}>
                {db.templates.map(t=>(
                  <button key={t.id} className="btn btn-sec btn-xs" onClick={()=>applyTemplate(t)}>{t.emoji} {t.name}</button>
                ))}
              </div>
            )}
            <div className="fg"><label>Nombre de la tienda *</label><input value={storeForm.name} onChange={e=>setStoreForm({...storeForm,name:e.target.value})} placeholder="Nombre completo"/></div>
            <div className="fg-row">
              <div className="fg"><label>Dirección *</label><input value={storeForm.address} onChange={e=>setStoreForm({...storeForm,address:e.target.value})} placeholder="Cra/Cll #-#, Barrio"/></div>
              <div className="fg"><label>Teléfono</label><input value={storeForm.phone} onChange={e=>setStoreForm({...storeForm,phone:e.target.value})} placeholder="604-xxx-xxxx"/></div>
            </div>
            <div className="fg-row">
              <div className="fg"><label>Categoría principal</label>
            ...
            <div className="fg-row-3">
              <div className="fg"><label>Emoji / Ícono</label><input value={storeForm.emoji} onChange={e=>setStoreForm({...storeForm,emoji:e.target.value})} placeholder="🏪" style={{fontSize:"1.5rem",textAlign:"center"}}/></div>
            ...
            <div className="fg-row">
              <div className="fg"><label>Latitud</label><input value={storeForm.lat} onChange={e=>setStoreForm({...storeForm,lat:e.target.value})} placeholder="6.2442"/></div>
              <div className="fg"><label>Longitud</label><input value={storeForm.lng} onChange={e=>setStoreForm({...storeForm,lng:e.target.value})} placeholder="-75.5812"/></div>
            </div>
            <div className="fg"><label>Descripción</label><textarea value={storeForm.description} onChange={e=>setStoreForm({...storeForm,description:e.target.value})} rows={2} placeholder="Breve descripción de la tienda..."/></div>
            <div style={{display:"flex",gap:8,marginTop:8}}>
...
export default function App() {
  ...
  const navClient = [
    ...
    {id:"map",icon:"🗺️",label:"Mapa / Tiendas"},
    {id:"orders",icon:"📋",label:"Mis Pedidos"},
  ];
  const navStore = [{id:"store",icon:"🏪",label:"Mi Tienda"}];
  const navAdmin = [
    {id:"admin",icon:"⚡",label:"Dashboard"},
    {id:"map",icon:"🗺️",label:"Mapa Global"},
  ];

  const navItems = user.role==="superadmin"?navAdmin:user.role==="store"?navStore:navClient;
  const pageTitle = {catalog:"Catálogo de Moda",map:"Mapa de Tiendas",orders:"Mis Pedidos",store:"Panel de Tienda",admin:"Superadmin"}[page]||page;
      ...
      <div className="shell">
        ...
        <aside className={`sidebar${sideOpen?" open":""}`}>
          ...
          <div className="nav-area">
            <div className="nav-sec-lbl">Navegación</div>
            {navItems.map(n=>(
              <button key={n.id} className={`nav-btn${page===n.id?" on":""}`} onClick={()=>{setPage(n.id);setSideOpen(false)}}>
                <span className="nav-ico">{n.icon}</span>{n.label}
              </button>
            ))}
            {user.role==="client" && (
              <>
                <div className="nav-sec-lbl" style={{marginTop:8}}>GPS</div>
          ...
          <div className="sidebar-foot">
            <div className="user-chip">
              <div className={`av ${user.role==="superadmin"?"av-cy":user.role==="store"?"av-or":"av-gr"}`}>
        ...
        <main className="main">
          <div className="topbar">
            ...
            {user.role==="client" && page!=="map" && (
              <button className="btn btn-sec btn-sm" onClick={detectLoc} style={{marginLeft:"auto"}}>
                {locLoading?<span className="spin">⏳</span>:"📍"} {locLoading?"Detectando...":"Ubicarme"}
              </button>
            )}
            {userLoc && user.role==="client" && <span className="badge b-ok" style={{marginLeft:page!=="map"?"4px":"auto"}}>📍 GPS ✓</span>}
            {user.role==="store" && (
              <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
                {(()=>{ const s=db.stores.find(st=>st.id===user.storeId); return s?<><span className="subdomain-pill" style={{fontSize:".65rem"}}>🌐 {s.subdomain}.distrimed.co</span><span className={`badge ${s.active?"b-ok":"b-off"}`}>{s.active?"ACTIVA":"INACTIVA"}</span></>:null; })()}
              </div>
            )}
          </div>

          <div className="content">
            {page==="catalog" && <Catalog user={user} db={db} setDb={setDb}/>}
